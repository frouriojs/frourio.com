...
By default, body is specified as string, and you can see the type error in `api/controller.ts`.

Then, specify `{ hello: 'world' }` to the body, and the type error should be resolved.
...
Also, the body of `api/hi` should be replaced as `{ hello: 'how are you?' }`.
...
Just to be sure, confirm that the server works as well as it did before.
...
3. Did you genearted server file? (`yarn frourio` in `frourio-tutorial/server` directory)
4. Did you generated api file? (`yarn aspida` in `frourio-tutorial` directory)

:::

## 2. Other Frourio Features {#other-frourio-features}

For more information on frourio features that were not covered in the tutorial, such as Hooks and Validation, please refer to [Docs](/docs).
