...
If defined in `hooks.ts`, it will be reflected in the current _and subordinate_ endpoints.

## Bridging Hooks and Controllers {#bridging-hooks-and-controllers}

To share values between hooks and controllers, extend and set them with `AdditinalRequest`.

### Example {#bridging-hooks-and-controllers-example}
