...
Frourio extends `FastifyRequest` type with the `AdditionalRequest` type exported from controller.ts and hooks.ts.

If defined in `controller.ts`, it will be reflected in the current endpoints.  
If defined in `hook.ts`, it will be reflected in the current _and subordinate_ endpoints.
