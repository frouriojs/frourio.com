...
In frourio, controllers are defined for filesystem-based API routing such as Next.js.
...
The level used for each difinition is different, but some can be defined at either level.
The following table summarizes them.

|                                                  |                                        Controller-level                                         |                              Directory-level                               |
| ------------------------------------------------ | :---------------------------------------------------------------------------------------------: | :------------------------------------------------------------------------: |
...
If defined at controller-level, it affects only the current endpoint.
...
<code>
  ...
  {',\n  handler: '}
  <a href="/docs/reference/handker#serverhandler">ServerHandler</a>
  {' or '}
  <a href="/docs/reference/handker#serverhandler">ServerHandler</a>Promise
  {',\n}'}
</code>

`validators`, `schemas` and `hooks` can be omitted.
If only `handler` is defined, [`ServerHandler`](#function-serverhandler) or <code><a href="#function-serverhandler">ServerHandler</a>Promise</code> can be passed directly instead of the object.

## Directory-level Hooks {#directory-level-hooks}

If defined at directory-level, it affects the current endpoint **and the endpoints under it**.
