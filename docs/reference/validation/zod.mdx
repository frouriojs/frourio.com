...
:::tip Looking for docs on how to define validators?

The definition of Validators can be found on the Routing page. See the following links for a reference on `defineController()` and `defineHooks()`.
...
The `validator` is an object whose keys are the same as `RequestParams` (`query`, `headers` or `body`) and whose values are **zod object schemas** (`z.object()`).
