...
```ts title='server/api/_bar@string/index.ts'
import { DefineMethods } from 'aspida';
import { Fuga, Baz } from './@types';
...
```ts title='server/api/_bar@string/controller.ts'
...
Frourio automatically creates `validatorsts` when a directory with a path parameter in its name is created.

:::

```ts title='server/api/_bar@string/validators.ts'
import { defineValidators } from './$relay';
import { z } from 'zod';

export default defineValidators(() => ({
  params: z.object({ bar: z.string() }),
}));
```
